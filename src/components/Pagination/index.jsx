        ...
        if (totalPages <= 10) {
            for (let i = 1; i <= totalPages; i++) {
                pageNumbers.push(
                    <li key={i} className={`page-item ${page === i ? 'active' : ''}`}>
                        <span className="page-link" onClick={() => setPage(i)}>
                            {i}
                        </span>
                    </li>
                );
            ...
            pageNumbers.push(
                <li key={totalPages - 1} className={`page-item`}>
                    <span className="page-link" onClick={() => setPage(totalPages - 1)}>
                        {totalPages - 1}
                    </span>
                </li>
            );
            pageNumbers.push(
                <li key={totalPages} className={`page-item`}>
                    <span className="page-link" onClick={() => setPage(totalPages)}>
                        {totalPages}
                    </span>
                </li>
            );
    ...
    return (
        <ul className="pagination">
            ...
            <li className={`page-item ${page === 1 ? 'disabled' : ''}`}>
                <div className="page-link" onClick={() => { if (page > 1) setPage(page - 1) }}>
            ...
            <li className={`page-item ${page === totalPages ? 'disabled' : ''}`}>
                <span className="page-link" onClick={() => { if (page < totalPages) setPage(page + 1) }}>
